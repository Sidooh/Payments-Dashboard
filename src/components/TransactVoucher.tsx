...
type CreditDebitMutation = MutationTrigger<
    MutationDefinition<
...
export const transactVoucher = async (
    action: Action,
    voucher: Voucher,
    credit: CreditDebitMutation,
    debit: CreditDebitMutation
) => {
    ...
    await Sweet.fire({
        ...
        confirmButtonText: action === 'credit' ? 'Credit' : 'Debit',
        ...
        preConfirm: async () => {
            ...
            if (formikRef?.isValid) {
                let res,
                    args = { id: voucher.id, ...formikRef?.values };
                if (action === 'credit') {
